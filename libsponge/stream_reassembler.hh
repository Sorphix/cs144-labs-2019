...
class StreamReassembler {
  private:
    ...
    // define the struct for the node of the red-black tree
    struct DataBlock {
      std::string data = ""; 
      size_t head_index = 0;
      size_t length = 0;
      // overwrite the < to set the behavior of set sorting
      bool operator<(const DataBlock &other) const {
        return head_index < other.head_index;
      }
    };
    // Red-Black Tree where the unassembled data blocks are sorted
    std::set<DataBlock> _blocks = {};

    ByteStream _output;  //!< The reassembled in-order byte stream
    size_t _capacity;    //!< The maximum number of bytes
    size_t _unassembled = 0; //!< The bytes uassembled
    size_t _head_index = 0;  //!< index of the head of the stream
    bool _eof_flag = false;

    // Merge db2 into db1
    long merge_blocks(DataBlock &db1, const DataBlock &db2);
